    ...
    for config_key in descriptor_data["configurations"].keys():
      ...
      for benchmark in benchmarks_org:
        ...
        with open(exp_path+config_key+'/bp.stat.0.csv') as f:
          lines = f.readlines()
          for line in lines:
            if 'BP_ON_PATH_CORRECT_count' in line:
              tokens = [x.strip() for x in line.split(',')]
              bp_exec = float(tokens[1])
              break
          for line in lines:
            if 'BP_ON_PATH_MISPREDICT_count' in line:
              tokens = [x.strip() for x in line.split(',')]
              bp_miss = float(tokens[1])
              break
        ...
        bp_config.append(bp_exec/(bp_miss + bp_exec))

      num = len(benchmarks)
      print(benchmarks)
      bp_config.append(avg_bp_exec/(avg_bp_miss + avg_bp_exec))
      bp[config_key] = bp_config

    benchmarks.append('Avg')
    plot_data(benchmarks, bp, 'Correct Branch Prediction Ratio', output_dir+'/BranchPrediction.png', ylim=(0.3, 1.12))
...
def plot_data(benchmarks, data, ylabel_name, fig_name, ylim=None):
  print(data)
  colors = ['#4363d8', '#911eb4', '#800000', '#f58231', '#3cb44b', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#e6beff', '#e6194b', '#000075', '#800000', '#9a6324', '#808080', '#ffffff', '#000000']
...
if __name__ == "__main__":
  ...
  parser.add_argument('-d','--descriptor_name', required=True, help='Experiment descriptor name. Usage: -d /home/$USER/lab3.json')
  ...
  get_BP(descriptor_data, args.simulation_path, args.output_dir)
  plt.grid('x')
  plt.tight_layout()
  plt.show()
